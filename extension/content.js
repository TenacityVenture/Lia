(() => {
  ...
  async function generateSpeech(text) {
    ...
    const response = await fetch('http://localhost:4000/api/tts/generate', {
      method: 'POST',
      headers: {
  ...
  async function handlereply_enabledant(commentInput) {
    ...
    try {
      ...
      if (error.message === "Failed to generate suggestions") {
        ...
        Error: ${error.message || "Failed to generate suggestions"}, maybe your session has expired. Please try signing in again, <a href='http://localhost:3000' target='_blank'>here</a>.
        </div>
        `
      } else {
      suggestionsContainer.innerHTML = `
  ...
  */
  ...
  async function enhanceNoteContent(type, content, instruction) {
    ...
    const response = await fetch("http://localhost:4000/api/note/enhance-note", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${await accessToken()}`,
  ...
  async function generateNoteTitle(content) {
    ...
    const response = await fetch("http://localhost:4000/api/note/generate-title", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${await accessToken()}`,
  ...
  async function sendMessage() {
    ...
    try {
      ...
      hideTypingIndicator()
      addMessageToChat("assistant", "Sorry, I encountered an error. Please try again. 😔\n Try Signin in again if the error continues - <a href='http://localhost:3000/login' target='_blank' style='color: blue'>here</a>")
  ...
  `
    ...
    try {
      ...
      try {
        ...
        addMessageToChat("assistant", "Unable to create new chat. Please check your connection or try signing in again <a href='http://localhost:3000/login' target='_blank'> here</a> ")
  ...
  async function loadChatHistory() {
    ...
    try {
      ...
      await refreshToken()
        ...
        addMessageToChat("assistant", "Unable to load conversations. Please check your connection or try signing in again <a href='http://localhost:3000/login' target='_blank' style='color:blue'> here </a>")
  ...
  function deleteConversation(id) {
    ...
    deleteChat()
      ...
      .catch((error) => {
        ...
        addMessageToChat("assistant", "Unable to delete conversation. Please check your connection or try signing in again <a href='http://localhost:3000/login' target='_blank'> here </a>")
      })
  };
  ...
  function renameConversation(id, newTitle) {
    ...
    renameChat()
      ...
      .catch((error) => {
        ...
        addMessageToChat("assistant", "Unable to rename conversation. Please check your connection or try signing in again <a href='http://localhost:3000/login' target='_blank'> here </a>")
      })
  }
  ...
  function formatLinks(text) {
    ...
    text = text.replace(/\[([^\]]+)\]\((http?:\/\/[^\s)]+)\)/g, (match, linkText, url) => {
    ...
    return text.replace(/(<a [^>]+>.*?<\/a>)|(\bhttp?:\/\/[^\s<]+)/g, (match, anchor, url) => {
