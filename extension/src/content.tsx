...
(() => {
  ...
  async function selectPersona(personaKey, container) {
    ...
    container
      .querySelector(`[data-persona="${personaKey}"]`)
      ...
      if (context.isReplyingToComment == true) {
        ...
        try {
          ...
          if (
            suggestions &&
            suggestions.error &&
            suggestions.error.includes("missing plan")
          ) {
            suggestionsContainer.innerHTML = `
            ...
            suggestions &&
            suggestions.error &&
            suggestions.error.includes("Plan expired")
          ) {
            suggestionsContainer.innerHTML = `
            <div style="color: red; padding: 10px;">
  ...
  async function handleRewriteAssistant(editor) {
      ...
      .map((p) => {
        ...
        p.childNodes.forEach((node) => {
          if (
            ...
            node.classList.contains("ql-mention")
          ) {
    ...
    if (!currentText.trim()) {
      ...
      showTemporaryMessage(
        editor,
        "Please write some text first to rewrite it",
      );
      return;
    }
    ...
    try {
      ...
      if (improvedText) {
        ...
        await window.animateTextRewriteWithMentions(
          editor,
          currentText,
          improvedText,
          improvements,
  ...
  function getCommentContext(commentInput) {
    ...
    if (window.location.href.includes("linkedin.com/pulse/")) {
      ...
      const articleHeaderTitle = document.querySelector(
      ...
      const commentsContainer = document.querySelector(
        ".reader-social-details__comments-list, .comments-comments-list",
      );
      if (commentsContainer) {
        const commentElementsContainer = commentsContainer.querySelectorAll(
          ".comments-comment-entity",
        );
        commentElementsContainer.forEach((comment) => {
          const commentElement = comment.querySelector(
            ".comments-comment-item__main-content",
          );
          const commenter = comment.querySelector(
      ...
      const articleWriterContainer = document.querySelector(
        ".reader-author-info__content",
      );
      if (articleWriterContainer) {
        const creatorFullname =
          articleWriterContainer.querySelector("div a").innerText;
      ...
      const commentMainContainer = commentInput.closest(
        ".comments-comment-entity",
      );
      let commentSocailActivity = null;
      if (commentMainContainer) {
        commentSocailActivity = commentMainContainer.querySelector(
          ".comment-social-activity",
        );
      } else {
        // we are in pulse article comments
        commentSocailActivity = commentInput.closest(
          ".comments-social-activity",
        );
      }
      ...
      if (commentSocailActivity) {
        ...
        const commentCommenterMeta = commentMainContainer.querySelector(
          ".comments-comment-meta__container .comments-comment-meta__description",
        );
        const commentContentElement = commentMainContainer.querySelector(
          ".comments-thread-entity",
        );

        // structure the context
        const commenterName = commentCommenterMeta.querySelector(
          ".comments-comment-meta__description-title",
        ).innerText;
        ...
        const currentContent =
          commentInput.querySelector(".ql-editor").textContent;
        ...
        const previousReplyContainer = commentSocailActivity.querySelector(
          ".comments-replies-list",
        );
        if (!previousReplyContainer) {
          // no one has replied to this comment yet
        ...
        const previousReplyElements = previousReplyContainer.querySelectorAll(
          ".comments-thread-entity",
        );
        previousReplyElements.forEach((reply) => {
          const replyElement = reply.querySelector(
            ".comments-comment-entity .comments-thread-entity",
          );
          const replier = reply.querySelector(
            ".comments-comment-entity .comments-comment-meta__container",
          );
      ...
      if (postContainer == null) {
        ...
        postContainer = commentInput.closest(
      ...
      const postCreatorContainer = postContainer.querySelector(
        ".update-components-actor__container",
      );
      const postCreator = postCreatorContainer.querySelector(
      ...
      const commentSocailActivity = commentInput.closest(
        ".comment-social-activity",
      );
      if (commentSocailActivity) {
        // we are in a comment input replying to a comment
  ...
  async function generateCommentSuggestions(context) {
    ...
    async function generate() {
      const response = await window.lia_fetchWithAuth(
        "https://api.getlia.live/api/prompt/suggest-reply",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async function generateReplyToCommentSuggestions(context) {
    ...
    if (
      context.previousRepliesOnComment &&
      context.previousRepliesOnComment.length > 0
    ) {
  ...
  function displayCommentSuggestions(container, suggestions, commentInput) {
    ...
    const suggestionElements = container.querySelectorAll(
      ".linkedin-ai-suggestion",
    );
  ...
  const PERSONAS = {
    professional: {
      ...
      icon: "💼",
      prompt:
        "Respond in a professional, formal tone suitable for business networking",
    },
    conversational: {
      name: "Conversational",
    ...
    thoughtLeader: {
      ...
      icon: "🧠",
      prompt:
        "Respond as a thought leader with insightful, authoritative, strategic perspectives",
    },
    supportive: {
      name: "Supportive",
    ...
    analytical: {
      ...
      icon: "📊",
      prompt:
        "Respond with analytical, data-driven insights and logical reasoning",
    },
    networking: {
      name: "Networking",
    ...
    conciseExpert: {
      ...
      icon: "🎯",
      prompt:
        "Respond as a concise expert: use minimal words, be direct, and deliver maximum insight in each reply",
    },
  };
  ...
  let TEMPLATES = {
    microContent: {
      ...
      prompt:
        "Write in micro-content style with very short sentences, each on a new line for maximum impact",
    },
    storyArc: {
      name: "Story Arc",
      ...
      prompt:
        "Structure your post as a story with a clear hook, context, challenge, resolution, and lesson learned",
    },
    listFormat: {
      name: "List Format",
      ...
      prompt:
        "Structure your content as a numbered list or bullet points with clear takeaways",
    },
    questionDriven: {
      ...
      prompt:
        "Start with a provocative question and build your narrative around answering it",
    },
    vulnerableLeader: {
      ...
      prompt:
        "Share a personal failure or struggle that led to growth, showing vulnerability and humanity",
    },
    contrarian: {
      ...
      prompt:
        "Present a contrarian viewpoint that challenges conventional wisdom in your industry",
    },
  };
